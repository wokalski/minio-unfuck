...
func NewXLStorage(ep Endpoint, cleanUp bool) (s *XlStorage, err error) {
	...
	if err != nil {
		fmt.Println("Invalid drive path", err)
		s.DrivePath = ep.Path
		return s, err
	}

	info, _, err := getDiskInfo(s.DrivePath)
	if err != nil {
		fmt.Println("No disk infoo: ", err)
		return s, err
	}
	...
	if err = makeFormatErasureMetaVolumes(s); err != nil {
		fmt.Println("make format something: ", err)
		return s, err
	}
	...
	if len(s.formatData) > 0 {
		...
		fmt.Printf("m: %d, n: %d, ep: %#+v, format: %#v\n", m, n, ep.URL.Path, ep.DiskIdx)
		fmt.Println("in another place")
		s.diskID = diskID
	...
	s.diskInfoCache.InitOnce(time.Second, cachevalue.Opts{},
		func(ctx context.Context) (DiskInfo, error) {
			...
			if err != nil {
				fmt.Println("initializing diskinfo cache: ", err)
				return dcinfo, err
			}
			...
			dcinfo.ID = diskID
			fmt.Println("something another error: ", err)
			return dcinfo, err
		},
	)
