...
func (j xlMetaV2Version) Valid() bool {
	...
	case LegacyType:
		return j.ObjectV1 != nil &&
			j.ObjectV1.valid()
	case ObjectType:
		fmt.Println(j.ObjectV2 != nil)
		fmt.Println(j.ObjectV2.ErasureAlgorithm.valid())
		fmt.Println(j.ObjectV2.BitrotChecksumAlgo.valid())
		fmt.Println(isXLMetaErasureInfoValid(j.ObjectV2.ErasureM, j.ObjectV2.ErasureN))
		fmt.Println(j.ObjectV2.ModTime > 0)
...
func (j xlMetaV2Object) ToFileInfo(volume, path string, allParts bool) (FileInfo, error) {
	fmt.Println("xlmetav20object tofileinfo called")
	versionID := ""
	var uv uuid.UUID
	// check if the version is not "null"
	if j.VersionID != uv {
