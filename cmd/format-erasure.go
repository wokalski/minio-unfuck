...
func loadFormatErasureAll(storageDisks []StorageAPI, heal bool) ([]*formatErasureV3, []error) {
	...
	fmt.Printf("format-erasure %#v\n", storageDisks)
	for index := range storageDisks {
		g.Go(func() error {
			if storageDisks[index] == nil {
				fmt.Println("errdisknotfound: ", index, errDiskNotFound)
...
func loadFormatErasure(disk StorageAPI, heal bool) (format *formatErasureV3, err error) {
	fmt.Println("LoadFormatErasure called")
	data, err := disk.ReadAll(context.TODO(), minioMetaBucket, formatConfigFile)
	if err != nil {
		fmt.Println("Error reading format.json: ", err)
	...
	if heal {
		...
		if err != nil {
			fmt.Println("load format can't heal: ", err)
			return nil, err
		}
		format.Info = info
	}
